...
from .provider import query_models_parallel, query_model, format_model_display_name
    ...
    for model_key, response in responses.items():
        if response is not None:  # Only include successful responses
            stage1_results.append(
                {
                    "model": model_key,  # e.g., "github/gpt-4o" or "openrouter/gpt-5.1"
    ...
    for model_key, response in responses.items():
        if response is not None:
            ...
            stage2_results.append(
                {"model": model_key, "ranking": full_text, "parsed_ranking": parsed}
            )
...
Provide a clear, well-reasoned final answer that represents the council's collective wisdom:"""
    ...
    response = await query_model(
        CHAIRMAN_MODEL["provider"], CHAIRMAN_MODEL["model"], messages
    )

    if response is None:
        # Fallback if chairman fails
        return {
            "model": f"{CHAIRMAN_MODEL['provider']}/{CHAIRMAN_MODEL['model']}",
...
Title:"""
    ...
    # Prefer GitHub models if available, fallback to OpenRouter
    title_model_config = CHAIRMAN_MODEL  # Use same as chairman for simplicity

    response = await query_model(
        title_model_config["provider"],
        title_model_config["model"],
        messages,
        timeout=30.0,
    )
